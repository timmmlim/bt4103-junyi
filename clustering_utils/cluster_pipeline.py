...
from clustering_utils import clustering_preprocessor
import time
...
class bt4103Cluster:
    ...
    def create_algo_input(self, input):
        self.input = input
        self.n_trials = len(input[2][1])
    ...
    def clustering_preprocessing(self, n_components=None):
        ...
        preprocessor.scale_numerical_data()
        preprocessor.perform_pca(n_components=n_components)
        preprocessor.pca_result()
        self.data = preprocessor.transformed_data
        # self.data.loc[:,:] = preprocessor.pca.inverse_transform(self.data)
        return "PCA performed"

    def initialize_algo_inputs(self):
        """
        ...
        self.input = [(KMeans, {'random_state': self.seed}, ('n_clusters', k_range)),
                      (GaussianMixture, {
    ...
    def update_cluster_param(input, current_k):
        """
    ...
    def run_clustering_pipeline(self):
        ...
        for model, params, n_trials_hp in self.input:
    ...
    def silhouette_analysis(self):
        ...
        for models in map(lambda x: x[0], self.input):
